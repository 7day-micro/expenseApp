...
from src.errors.expense import NoExpenseFound, ExpenseCreationDataBase, ExpenseDeletionDataBase
from src.common.base_service import BaseService
from src.domain.expense.schemas import ExpenseSchema, ExpenseCreateSchema, ExpenseResponseSchema
...
class ExpenseService(BaseService[Expense, ExpenseCreateSchema, ExpenseResponseSchema]):
    ...
    async def get_by_id(self, object_id: Any, user_id: UUID) -> Optional[Expense]:
        statement = select(Expense).where(Expense.id == object_id, Expense.user_id == user_id)
