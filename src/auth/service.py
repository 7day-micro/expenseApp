...
async def login_oauth(user_credentials: OAuth2PasswordRequestForm, db: AsyncSession):
    ...
    if not user or not oauth2.verify_password(
        ...
        raise HTTPException(
            ...
            detail=f"Invalid credentials for email: {user_credentials.username}",
        )
...
async def refresh_token_service(refresh_token: str, db: AsyncSession):
    ...
    try:
        payload = jwt.decode(
            refresh_token, oauth2.SECRET_KEY, algorithms=[oauth2.ALGORITHM]
        )
...
async def get_current_user(token_schema, db: AsyncSession):
    return await oauth2.get_current_user(token_schema, db)
